...
const deletePost = async(req, res) => {
  try {
    ...
    const post = await Post.findOne({_id: id})  
    if(!existEmail){
      return res.status(400).json({state:false, msg: `You are not authorized to delete the score`})
    }
    else{
      if(!existEmail.isVerified){
        return res.status(400).json({state: false, msg: `Your account is not verified. You are enable to delete your score`})
      }
      else{
        if(!post){
          return res.status(400).json({state: false, msg: `Post does not exist`})
          ...
          return res.status(200).json({state:true,msg:`Score is deleted successfully`})
        }
      }
    }
  } 
